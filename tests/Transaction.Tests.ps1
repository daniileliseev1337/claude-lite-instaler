It 'installs active files and never copies quarantine into discovery paths' {
  ...
  try {
    ...
    Assert-True (Test-Path -LiteralPath (Join-Path $Scenario.user_profile '.agents\skills\sample-skill\SKILL.md'))
    Assert-False (Test-Path -LiteralPath (Join-Path $Scenario.user_profile '.codex\mcp'))
    Assert-False (Test-PendingFoundationJournal $Scenario.local_app_data)
...
It 'leaves a recoverable journal after injected replacement failure' {
  ...
  try {
    ...
    Assert-True (Test-PendingFoundationJournal $Scenario.local_app_data)
    $Rollback = Invoke-FoundationRollback $Scenario.user_profile $Scenario.local_app_data
...
It 'treats an idempotent reinstall as a verified no-op' {
  ...
  try {
    ...
    $Again = New-FoundationPlan $Scenario.package_root $Scenario.user_profile `
      $Scenario.local_app_data $Scenario.environment
    Assert-Equal 2 @($Again.rows | Where-Object action -eq 'UNCHANGED').Count
